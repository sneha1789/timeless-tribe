...
const NewsletterSlide = ({ showNotification }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!showNotification) return; // Guard clause
...
const ServicesSlide = ({ showNotification }) => {
...
const TestimonialsSlide = (
  { latestReviews, loadingReviews, errorReviews, showNotification }, // Accept prop
) => (
  <div className="carousel-slide">
...
const Home = () => {
  ...
  // --- FIX 1: Add state for notifications ---
  ...
  // --- FIX 2: Define the showNotification function ---
    ...
    const fetchPageData = async () => {
      try {
        ...
        if (Array.isArray(promotionsData)) {
          ...
          console.error('API did not return an array for promotions.');
          setPromotions([]);
        }
      } catch (error) {
